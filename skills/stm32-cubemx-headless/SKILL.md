...
> agent 的文件工具写不进去（`Access to the path ... is denied`）。因此固定如下分工：
>
> | 步骤 | 怎么做 |
> |---|---|
> | 生成工程 | `stm32_generate`（插件进程写入，不受限） |
> | **写业务代码** | **`stm32_user_code`** —— 只写 `USER CODE BEGIN/END` 之间，区外一个字节不动 |
> | 编译 / 烧录 | **用户手动执行**，agent 不代做 |
>
> 也正因如此，**不需要**把会话工作区改成 `D:\STM32_Workspace`。

> **交接边界**：本技能负责到「生成工程 + 写代码」为止。**编译与烧录由用户手动完成**——
> 不要代为调用 Keil/构建脚本/烧录器，只把工程路径与命令交给用户。
