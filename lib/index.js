 ...
 */
import { existsSync, readdirSync, readFileSync, writeFileSync, appendFileSync, mkdirSync, copyFileSync, statSync } from 'node:fs'
...
import * as usercode from './usercode.js'
import * as appfile from './appfile.js'
...
/**
 * 当前平台描述符与工具链探测 —— 所有与操作系统有关的判断都走 lib/platform.js，
 * 本文件里不再出现 `.exe` / `C:\` / `powershell` 之类的硬编码。
 */
const PF = platform.resolvePlatform()
const HOME = PF.home
const BUNDLES = platform.bundlesDir(PF)

/**
 * 工程代码根（由用户指定）。stm32_generate 未显式给 outputDir 时默认落在这里，
 * 以便所有 agent 产出的工程集中在一处、互不污染既有工程。
 * Windows 保持本机约定 D:\STM32_Workspace\DSHCode；POSIX 落在 ~/STM32_Workspace/DSHCode。
 * 两种情况都可用环境变量 DSH_STM32_CODE_ROOT 覆盖。
 */
const DEFAULT_CODE_ROOT = platform.defaultCodeRoot(PF)

const firstExisting = (paths) => paths.find((p) => p && existsSync(p)) ?? null

/** 工具链与 CubeMX 的探测全部委托给平台层（Windows/POSIX 的候选表在那里）。 */
function detectToolchains() {
  return platform.detectToolchains(PF)
}

function detectCubeMx(explicit) {
  return platform.detectCubeMx(PF, explicit)
}

function detectFirmwarePacks() {
  const repo = platform.firmwareRepo(PF)
...
/** 串口枚举：Windows 走 PowerShell + .NET，POSIX 读 /dev（见 serial.js / platform.js）。 */
async function listSerialPorts() {
  try {
    return await serial.listPorts({ platform: PF })
  } catch {
    return null
  }
}
...
async function listProbes(programmerCli) {
  ...
  try {
    const { stdout } = await execFileAsync(programmerCli, ['-l'], { timeout: 60000, ...platform.spawnOpts(PF), maxBuffer: 8 << 20 })
    const found = []
...
function ensureMdkCompiler(projectDir, keilUv4) {
  // Keil 只在 Windows 上运行：POSIX 即便工程里有 MDK-ARM/（CubeMX 在哪都能生成 MDK 工程），
  // 也没有 UV4/ARMCLANG 可用，直接说明并跳过，不要报成"探测失败"。
  if (!PF.isWindows) {
    return { changed: false, reason: 'Keil MDK（UV4/ARMCLANG）仅 Windows 可用，跳过 MDK 编译器修正' }
  }
...
/**
 * 总开关：默认**关闭**。关闭时不是"工具自己拒绝"，而是把工具从模型的工具表里移除，
 * 模型根本看不到、也就不会调用。开启后立即恢复。
 *
 * 状态存 ~/.dsh/dsh-stm32.json；同端口有开关页：http://127.0.0.1:13080/stm32/
 */
const FLAG_FILE = join(process.env.DSH_HOME ?? join(HOME, '.dsh'), 'dsh-stm32.json')
const PANEL_HTML = new URL('./panel.html', import.meta.url)
const DEBUG_LOG = join(process.env.DSH_HOME ?? join(HOME, '.dsh'), 'dsh-stm32-debug.log')

const TOOL_NAMES = new Set([
  'stm32_env', 'stm32_ioc_read', 'stm32_ioc_set', 'stm32_generate',
  'stm32_guard', 'stm32_flash', 'stm32_serial', 'stm32_user_code',
  'stm32_app_file',
])

function readEnabled() {
  try { return JSON.parse(readFileSync(FLAG_FILE, 'utf8')).enabled === true } catch { return false }
}
function writeEnabled(v) {
  mkdirSync(dirname(FLAG_FILE), { recursive: true })
  writeFileSync(FLAG_FILE, JSON.stringify({ enabled: !!v }, null, 2) + '\n', 'utf8')
  return !!v
}
function statePayload() {
  return { ok: true, enabled: readEnabled(), tools: [...TOOL_NAMES], flagFile: FLAG_FILE }
}
...
export function apply(ctx) {
  ...
  ctx.effect(() => ctx.tools.register(defineTool({
    ...
    parameters: {
      cubeMxPath: { type: 'string', description: `显式指定 STM32CubeMX 可执行文件路径（可选）。${PF.isMac ? 'macOS 上是 STM32CubeMX.app/Contents/MacOS/STM32CubeMX' : PF.isWindows ? 'Windows 上是 STM32CubeMX.exe' : 'Linux 上是解压目录里的 STM32CubeMX'}`, },
      probeHardware: { type: 'boolean', description: '是否真的调用 STM32_Programmer_CLI 枚举探针（较慢，约数秒）' },
    },
    isConcurrencySafe: () => true,
    output: jsonOut,
    async execute(args) {
      // 总开关兜底：即使模型绕过工具表调用（或开关在会话中途被关掉），也一律拒绝。
      if (!readEnabled()) {
        return j({ ok: false, disabled: true, hint: 'STM32 助手总开关未开启：打开 http://127.0.0.1:13080/stm32/ 点一下即可' })
      }
      ...
      const out = {
        platform: {
          ...
          verifiedNote: PF.verified
            ? undefined
            : `本平台（${PF.label}）的代码路径已实现并有注入式单测，但未经真机验证——报告结论时请如实标注。`,
          defaultCodeRoot: DEFAULT_CODE_ROOT,
          pathDelimiter: PF.pathDelimiter,
        },
      ...
      const missing = []
      if (!out.cubeMx.exists) missing.push(`STM32CubeMX（stm32_generate 必需）${out.cubeMx.installHint ? '：' + out.cubeMx.installHint : ''}`)
      if (PF.isWindows && !out.toolchains.keilUv4) missing.push('Keil UV4.exe（build-keil 技能必需）')
      if (!out.toolchains.armGcc) missing.push('arm-none-eabi-gcc')
      if (!out.toolchains.make) missing.push(PF.isWindows ? 'mingw32-make（GNU Make 构建路径所需）' : 'make / gmake（GNU Make 构建路径所需）')
      if (!out.toolchains.cmake) missing.push('cmake（CMake 构建路径所需）')
      if (!out.toolchains.programmerCli && !out.toolchains.openocd && !out.toolchains.stflash) {
        missing.push('烧录器 CLI（STM32_Programmer_CLI / openocd / st-flash 任一）')
      }
  ...
  ctx.effect(() => ctx.tools.register(defineTool({
    ...
    parameters: {
      ...
      outputDir: { type: 'string', description: `生成目录；**缺省 = ${DEFAULT_CODE_ROOT}**（本机约定的代码根）。仅当用户明确要求其他位置时才传。` },
    ...
    async execute(args) {
      ...
      if (args.acknowledgeOutsideSandbox !== true) {
        return j({
          ...
          whatItTouches: platform.generateTouchList(PF, detectCubeMx().path),
      ...
      if (!cubeMx.exists) return j({ ok: false, error: `找不到 STM32CubeMX（探测: ${cubeMx.path ?? '无'}）`, hint: cubeMx.installHint, platform: PF.id })
  ...
  ctx.effect(() => ctx.tools.register(defineTool({
    name: 'stm32_flash',
    timeoutMs: 180000,
    description: '生成烧录命令交给用户手动执行（默认只给命令，不代为烧录）。仅当用户明确要求代为烧录时才传 mode=auto。',
    parameters: {
      file: { type: 'string', required: true, description: '.elf/.hex/.bin 路径' },
      mode: { type: 'string', enum: ['auto', 'manual'], description: '默认 manual（只返回命令，不执行）。烧录由用户手动完成；仅用户明确要求时才传 auto' },
    ...
    async execute(args) {
      ...
      const cli = tc.programmerCli
      const file = resolve(args.file)
      if (!existsSync(file)) return j({ ok: false, error: `找不到固件文件: ${file}` })

      if (!cli) {
        // 没有官方 CLI 时，给出备选后端的**模板命令**（只给命令、不自动执行，
        // 且明确标注未真机验证——不要拿它当"已实测可用"）。
        const alternatives = []
        if (tc.openocd) alternatives.push({ backend: 'openocd', command: flash.buildOpenOcdCommand({ file, address: args.address }) })
        if (tc.stflash) alternatives.push({ backend: 'st-flash', command: flash.buildStFlashCommand({ file, address: args.address }) })
        return j({
          ok: false, platform: PF.id,
          error: '找不到 STM32_Programmer_CLI（STM32CubeProgrammer 的命令行）',
          installHint: PF.isMac
            ? '装 STM32CubeProgrammer.app（CLI 在 Contents/MacOS/bin/ 下），或 brew install stlink / openocd。'
            : '装 STM32CubeProgrammer（Linux 官方 .zip 解压后把 bin/ 加进 PATH），或 apt/dnf 装 stlink-tools / openocd。',
          alternatives,
          note: alternatives.length
            ? '备选后端命令是**模板**：本仓库未在真机上验证过它们的写入行为，请自行核对参数。'
            : undefined,
        })
      }
      ...
      try {
        const r = await execFileAsync(cli, cliArgs, { timeout: 150000, ...platform.spawnOpts(PF), maxBuffer: 32 << 20 })
        stdout = r.stdout; stderr = r.stderr; code = 0
      } catch (e) {
  ...
  ctx.effect(() => ctx.tools.register(defineTool({
    ...
    async execute(args) {
      ...
      const ports = await serial.listPorts({ platform: PF })
      if (args.listOnly || !args.port) {
        return j({
          ok: true, ports, count: ports.length,
          backend: platform.serialBackend(PF),
          note: ports.length ? undefined : (PF.isWindows
            ? '当前没有串口设备（板子未插或驱动未装）。'
            : '当前没有串口设备。检查：板子已插、驱动已装（CH340/CP210x 常用）、设备名形如 /dev/ttyUSB0 或 /dev/cu.usbserial-*。'),
        })
      }
      ...
      if (!r.ok) return j({ ok: false, error: r.error, hint: r.hint, captured: r.captured, port: args.port, backend: r.backend })
      const m = serial.matchExpectation(r.captured, args.expect)
      return j({
        ok: m.matched === false ? false : true,
        port: args.port, baud: args.baud ?? 115200,
        backend: r.backend,
        platformVerified: PF.verified,
  ...
  // ── 8. 只往 USER CODE 区写代码 ─────────────────────────────────────────────
  ctx.effect(() => ctx.tools.register(defineTool({
    name: 'stm32_user_code',
    description: '把业务代码写进 CubeMX 的 USER CODE 保留区。⚠️ 在会话沙箱外写入，但结构性限制为只能写区标记之间（区外一个字节不动）。action=list 可查看可用区域。',
    parameters: {
      file: { type: 'string', required: true, description: 'C/H 源文件路径，如 <工程>/Core/Src/main.c' },
      action: { type: 'string', enum: ['list', 'write'], description: '缺省 write' },
      block: { type: 'string', description: '区域名，如 main.c 的 2 / 3 / WHILE / Includes / PV / PFP' },
      code: { type: 'string', description: '要写入的代码（逐字插入，不自动缩进）' },
      mode: { type: 'string', enum: ['replace', 'append'], description: '缺省 replace；append 追加在既有内容之后' },
    },
    output: jsonOut,
    async execute(args) {
      // 总开关兜底：即使模型绕过工具表调用（或开关在会话中途被关掉），也一律拒绝。
      ...
      const file = resolve(args.file)
      try {
        if ((args.action ?? 'write') === 'list') return j({ ok: true, ...usercode.readBlocks(file) })
        if (!args.block) return j({ ok: false, error: 'write 需要 block（区域名）。先 action=list 看有哪些可用。' })
        if (args.code == null) return j({ ok: false, error: 'write 需要 code' })
        return j(usercode.writeBlock(file, args.block, args.code, { mode: args.mode ?? 'replace' }))
      } catch (e) {
        return j({ ok: false, error: String(e?.message ?? e) })
      }
    },
  })), '@dsh-external/dsh-stm32: user code')

  // ── 9. 总开关：关闭时把工具从模型工具表移除 ────────────────────────────────
  // system-prompt/assemble 是 Waterfall：必须 await next() 后再裁剪，否则会吃掉下游段落。
  ctx.on('system-prompt/assemble', async (_assembly, _context, next) => {
    const assembled = await next()
    try {
      const enabled = readEnabled()
      // 工具项的形状不一定是 {name}：可能是 OpenAI 风格的 {type:'function',function:{name}}。
      // 用容错取值，否则 name 恒为 undefined → 一个都滤不掉（实测踩过）。
      const nameOf = (t) => t?.name ?? t?.function?.name ?? t?.tool?.name ?? t?.id
      const before = Array.isArray(assembled?.tools) ? assembled.tools.length : -1
      const kept = Array.isArray(assembled?.tools)
        ? (enabled ? assembled.tools : assembled.tools.filter((t) => !TOOL_NAMES.has(nameOf(t))))
        : assembled?.tools
      const after = Array.isArray(kept) ? kept.length : -1
      // 调试记录：确认钩子是否触发、工具项真实形状（只记首项，截断）
      try {
        const probe = Array.isArray(assembled?.tools) && assembled.tools[0]
          ? JSON.stringify(assembled.tools[0]).slice(0, 300)
          : '(无 tools 数组)'
        appendFileSync(DEBUG_LOG, new Date().toISOString() +
          ' enabled=' + enabled + ' tools=' + before + ' kept=' + after +
          ' keys=' + JSON.stringify(Object.keys(assembled ?? {})).slice(0, 200) +
          ' first=' + probe + '\n', 'utf8')
      } catch { /* 忽略 */ }
      return { ...assembled, tools: kept }
    } catch {
      return assembled // 裁剪失败就退回原样，绝不因为开关把会话搞坏
    }
  })

  // ── 10. 开关页 + API（与 DSH Web 同源同端口） ──────────────────────────────
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        const p = url.pathname.replace(/^\/stm32/, '') || '/'
        if (req.method === 'GET' && (p === '/' || p === '/index.html')) {
          return send(200, readFileSync(PANEL_HTML, 'utf8'), 'text/html; charset=utf-8')
        }
        if (req.method === 'GET' && p === '/api/state') return send(200, statePayload())
        if (req.method === 'POST' && p === '/api/enable') {
          const body = JSON.parse((await readBody(req)) || '{}')
          if (typeof body.enabled !== 'boolean') return send(400, { ok: false, error: 'enabled 必须是布尔值' })
          writeEnabled(body.enabled)
          return send(200, statePayload())
        }
  ...
  }), 'dsh-stm32: toggle panel + api')

  // ── 11. 斜杠命令：DSH 界面内的入口（输入框打 / 即可看到并选中） ──────────────
  ctx.effect(() => ctx.commands.register({
    name: 'stm32',
    description: 'STM32 助手总开关：/stm32 on | off | status',
    ...
    handler: ({ rawInput }) => {
      ...
      if (arg === '' || arg === 'status') {
        return {
          kind: 'success',
          text: 'STM32 助手当前' + (readEnabled() ? '已开启' : '已关闭') +
            '（默认关闭）。切换：/stm32 on 或 /stm32 off',
        }
      }
      if (arg === 'on' || arg === 'off') {
        const on = arg === 'on'
        ...
        return {
          kind: 'success',
          text: on
            ? 'STM32 助手已开启：8 个 stm32_* 工具已可用（本会话如未出现，新开一个会话即可）。'
            : 'STM32 助手已关闭：stm32_* 工具已从模型工具表移除，不会被调用。',
        }
      }
      return { kind: 'error', text: '未知参数 "' + arg + '"。用法：/stm32 on | off | status' }
    },
  }), 'dsh-stm32: slash command')

  // ── 12. App/ 自建模块文件（作用域限定在 <工程>/App/） ──────────────────────
  ctx.effect(() => ctx.tools.register(defineTool({
    name: 'stm32_app_file',
    description: '在工程 App/ 下新建/读取/删除自建模块文件（结构性限定在 App/ 内），并自动按工程里存在的构建文件登记：Keil 分组与 IncludePath / Makefile 的 C_SOURCES 与 C_INCLUDES / CMake 的用户区 target_sources。',
    parameters: {
      projectRoot: { type: 'string', required: true, description: 'CubeMX 工程根目录（含 Core/）' },
      action: { type: 'string', enum: ['write', 'read', 'list', 'delete'], description: '缺省 write' },
      file: { type: 'string', description: 'App/ 下的相对路径，如 motor.c 或 pid/pid.h' },
      code: { type: 'string', description: 'write 时的文件内容' },
      register: { type: 'boolean', description: '是否登记进 Keil 工程（缺省 true；仅 .c 需要）' },
    },
    output: jsonOut,
    async execute(args) {
      if (!readEnabled()) {
      ...
      try {
        const act = args.action ?? 'write'
        if (act === 'list') return j({ ok: true, ...appfile.listApp(args.projectRoot) })
        if (act === 'read') return j({ ok: true, ...appfile.readApp(args.projectRoot, args.file) })
        if (act === 'delete') return j(appfile.deleteApp(args.projectRoot, args.file))
        if (args.code == null) return j({ ok: false, error: 'write 需要 code 参数' })
        return j(appfile.writeApp(args.projectRoot, args.file, args.code, { register: args.register !== false }))
      } catch (e) {
        return j({ ok: false, error: String(e?.message ?? e) })
      }
    },
  })), 'dsh-stm32: app module file')
}
