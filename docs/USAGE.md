...
1. **在 DSH 输入框里打 `/`，找到 `stm32`** —— 直接在界面里，不用离开：
   - `/stm32 on` 开启 · `/stm32 off` 关闭 · `/stm32 status` 查看当前状态
2. 开关页（与 DSH Web 同端口、同源）：<http://127.0.0.1:13080/stm32/> —— 一个按钮，点一下即切
3. 直接对 agent 说「打开 STM32 开关」

切换立即生效；若当前会话的工具表还没刷新，新开一个会话即可。
